...
| 层级 | 内容 |
|------|------|
| 度量 | RUM 大盘 P75/P95，按页面/端/地区分维度 |
| 预算 | JS <300KB gzip、LCP <2.5s、INP <200ms 写入 CI |
| 门禁 | Lighthouse CI + bundle analyzer PR 检查 |
| 治理 | 季度专项、性能 Owner、回归复盘 |
...
1. **概念**：三大指标分别衡量什么？→ LCP 加载、INP 交互、CLS 稳定。
2. **机制**：LCP 由什么组成？→ TTFB + 资源加载 + 元素渲染，用 attribution 定位。
3. **边界**：INP 和 FID 区别？→ INP 观测全页交互 P98，FID 仅首次输入（已废弃）。
4. **应用**：CLS 怎么优化？→ 图片设尺寸、字体 font-display、动态内容预留空间。
5. ⭐ **原理（触底）**：线上 LCP P75 偏高，你怎么系统性归因到具体修复项？→ RUM attribution 拆 TTFB/资源/渲染四段定位瓶颈段 → 对应优化（CDN/preload/SSR/字体）→ 设性能预算门禁 → 灰度验证 P75 回落。
6. **实战（落地）**：你真做过一次 LCP 优化吗？讲讲完整链路。→ 场景：详情页 LCP P75=5.2s 且集中在 4G 用户；步骤：DevTools 录制定位 LCP 元素是 2.3MB banner PNG，四段式发现 Load Time 占 3.5s 且无 preload → 改 WebP(280KB) + `preload fetchpriority=high` + 响应式 srcset；验证：Lighthouse 5.2s→1.9s，灰度一周后 RUM P75→2.0s；结果：详情页跳出率下降、转化提升，并把该图纳入性能预算防回退。

### 链二：度量体系

1. **概念**：Lab 和 Field 区别？→ Lighthouse 实验室、RUM 真实用户 P75。
2. **机制**：为什么看 P75 而非平均？→ 平均被极值拉偏，分位数反映多数用户体验。
3. **应用**：怎么建性能治理闭环？→ 采集基线 → 预算门禁 → 排优先级 → 修复 → 验证 → 监控。
4. ⭐ **原理（触底）**：怎么做到「性能不退化」而非一次性优化？→ CI 接入 Lighthouse/bundlesize 门禁 + 线上 RUM 大盘告警 + 新需求评审纳入性能预算，把性能变成流程而非运动。
5. **实战（落地）**：你怎么把性能治理真正落进团队流程？→ 场景：性能靠运动式优化、上线即回退；步骤：接入 web-vitals 上报建 P75 基线看板 → 写 `lighthouse-budget.json` 接 Lighthouse CI + bundlesize 门禁 → 新需求评审纳入预算 → 按流量×严重度排期；验证：每个性能 PR 必带 before/after，超预算 CI 直接 fail；结果：核心页 P75 季度内稳定下降且不再偷偷回退。
