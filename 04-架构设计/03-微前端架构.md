...
qiankun 基于 single-spa：注册 app、匹配 activeRule、加载 entry、调用 bootstrap/mount/unmount 生命周期。
...
| 方案 | 原理 | 缺点 |
|------|------|------|
| 快照 | 保存/恢复 window 属性 | 性能、不完整 |
| Proxy | 代理 window 读写 | 需处理 with 等 |
| 多实例 | iframe + Proxy | 内存开销 |

### 9. 样式隔离与通信

**样式隔离：** Shadow DOM、`scoped CSS`、CSS Modules 前缀、`:where()` 降低特异性。

**通信方式：**

```javascript
// 1. props 下发（主应用 → 子应用）
mount(props) { props.onGlobalStateChange(state => {}); }
...
### 10. 公共依赖与 wujie / micro-app

- **共享依赖：** webpack externals / Module Federation 避免重复打包 react
- **wujie：** iframe + WebComponent，天然 JS/CSS 隔离
- **micro-app：** 类 WebComponent 容器，简化接入

各方案缺点：qiankun 样式/沙箱边界 case；MF 绑定构建工具；iframe 性能与体验；需按团队选型。
