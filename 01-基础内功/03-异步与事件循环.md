...
Promise.myAll = (promises) => new Promise((resolve, reject) => {
  ...
  promises.forEach((p, i) => Promise.resolve(p).then(
    v => { res[i] = v; if (++n === promises.length) resolve(res); },
    reject
  ));
});
...
> 大厂对一个考点通常追问到能力边界：**概念 → 机制 → 边界 → ⭐ 原理（触底）→ 实战（落地）**。实战层是区分「背过」和「做过」的关键。事件循环常一路追到输出题。

### 链一：事件循环（必问，常追 5-6 层）

1. **概念**：讲讲事件循环？→ 执行同步栈 → 清空微任务队列 → 取一个宏任务 → 再清微任务 → 循环。
2. **机制**：宏任务和微任务分别有哪些？→ 宏：setTimeout/setInterval/MessageChannel/IO；微：Promise.then/queueMicrotask/MutationObserver。
3. **边界**：微任务什么时候清空？→ 每个宏任务结束后、渲染前，把当前微任务队列**全部**清空（包括执行中新产生的）。
4. **输出题**：给一段 `setTimeout + Promise + async` 代码说顺序（见本章输出题集）。
5. **边界**：Node 和浏览器差异？→ Node 有 process.nextTick（优先于 Promise）、setImmediate；分 timers/poll/check 多阶段。
6. ⭐ **原理（触底）**：为什么要区分宏/微任务？→ 微任务保证一批同步逻辑后、渲染前原子完成（如 Promise 链、Vue nextTick），既及时又不打断渲染节奏。
7. **实战（落地）**：线上偶发「状态更新后 UI 没刷」怎么排查？→ console 打微任务顺序复现；查是否宏任务里漏 await、Promise 链未 catch；改 queueMicrotask/nextTick 对齐更新时机，复测用户操作路径。

### 链二：async/await

1. **概念**：async/await 本质？→ Generator + 自动执行器（co）的语法糖，返回 Promise。
2. **机制**：await 会阻塞吗？→ 不阻塞主线程，只暂停当前 async 函数，后续代码进微任务。
3. **边界**：多个 await 串行 vs 并行？→ 无依赖应 `Promise.all` 并行，避免串行累加耗时。
4. ⭐ **原理（触底）**：try/catch 能捕获 await 的 reject 吗？未 await 的 Promise reject 呢？→ 能捕获 await 的；游离 Promise 的 rejection 需 `.catch` 或全局 unhandledrejection，否则丢失。
5. **实战（落地）**：接口层 async 错误怎么统一处理？→ 封装 request 统一 try/catch + 全局 unhandledrejection 上报；Sentry 聚合 Promise rejection；改完后故意抛错验证监控能抓到。

### 链三：并发与竞态

1. **概念**：Promise.all / race / allSettled / any 区别？→ all 全成功/一败全败；race 最快 settle；allSettled 全完成返回状态；any 第一个成功。
2. **应用**：怎么做并发限流？→ 维护执行集合，达上限 `Promise.race` 等一个完成再补（见本章 pool）。
3. **边界**：搜索请求竞态（后发先到）怎么解？→ 请求序号比对/AbortController 取消旧请求/用 Query 库。
4. ⭐ **原理（触底）**：AbortController 能真正停止请求吗？→ 能中断 fetch 网络层并 reject，但已发出的服务端处理不一定回滚；前端只保证不采用旧结果。
5. **实战（落地）**：搜索框竞态怎么落地？→ 每次输入递增 requestId，响应回来比对 id 不匹配则丢弃；或 AbortController 取消上一次 fetch；快速连打关键词验证只显示最后一次结果。
