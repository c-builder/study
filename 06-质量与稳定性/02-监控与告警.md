...
```javascript
// 1. JS 运行时错误
window.onerror = (msg, url, line, col, error) => { report(error); };
...
// 4. 跨域脚本 — script 需 crossorigin 才能拿到堆栈
<script src="https://cdn.example.com/app.js" crossorigin></script>
```

### 7. Source Map 安全与 Session Replay

- 上传 map 到 Sentry 私有存储，**不要**部署到公网 CDN
- **Session Replay：** 录制 DOM 变更（需脱敏 input），复现用户操作路径
...
function report(payload) {
  if (navigator.sendBeacon) {
    ...
    fetch('/api/log', { method: 'POST', body: JSON.stringify(payload), keepalive: true });
  }
}
// 合并批量上报、离线队列、采样率
```

**白屏检测：** 监听 `load` 后检测 `#root` 是否有子节点，或 MutationObserver 超时无内容。
