...
HTTPS 加密传输，防窃听与篡改；HSTS 强制浏览器仅用 HTTPS，防 SSL stripping。

### 9. iframe sandbox 与 postMessage 安全
...
window.addEventListener('message', (e) => {
  ...
  // 校验 e.data 结构，勿直接 eval
});
```

### 10. 第三方脚本与 JWT / OAuth

**第三方脚本：** 广告/统计 SDK 可读写页面，用 SRI、CSP `script-src` 限制来源。

**JWT：** 无状态 token，**不要**存敏感信息在 payload（Base64 可解码）；短过期 + refresh token。

**OAuth 2.0 / OIDC：** 授权码模式 + PKCE（SPA）；state 防 CSRF；redirect_uri 白名单。
...
前端配合：接口返回 403、UI 按权限隐藏；**真正校验在后端**。

**GDPR：**  cookie  consent、数据导出/删除、隐私政策；前端埋点需用户同意。
