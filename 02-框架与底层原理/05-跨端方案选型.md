...
> 原理深挖见 [07-跨端原理深挖](./07-跨端原理深挖.md)（双线程、Bridge/JSI、JSBridge 手写）。

## 面试高频问答（追问链）

> 大厂对一个考点通常追问到能力边界：**概念 → 机制 → 边界 → ⭐ 原理（触底）→ 实战（落地）**。实战层是区分「背过」和「做过」的关键。原理深挖见 [07-跨端原理深挖](./07-跨端原理深挖.md)。

### 链一：跨端选型

1. **概念**：跨端方案分几类？→ Web 系（小程序/H5）、Native 系（RN/Flutter）、桌面系（Electron/Tauri）。
2. **机制**：小程序为什么 setData 慢？→ 逻辑层与视图层双线程通信需序列化。
3. **边界**：Taro 和 uni-app 区别？→ 都编译跨端，差在生态、语法偏好、支持端，受最小公约数限制。
4. **应用**：RN 和 Flutter 怎么选？→ RN 映射原生、复用 React 栈；Flutter 自绘一致性高、性能稳。
5. ⭐ **原理（触底）**：RN Bridge 为什么是性能瓶颈？新架构 JSI/Fabric 怎么解决？→ Bridge 异步 + JSON 序列化跨线程；JSI 让 JS 直接持有 C++ 对象同步调用，Fabric 统一渲染管线、TurboModules 按需加载。
6. **实战（落地）**：小程序+H5 双端怎么选型落地？→ 团队 React 栈选 Taro、要原生体验选 RN/Flutter；POC 对比 setData/首屏/包体；ADR 记录 trade-off，按端独立发版与监控。
